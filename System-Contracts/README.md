# SilverBitcoin RPC System Contracts
This repository hosts system smart contracts of the SilverBitcoin RPC Chain

# Consensus
`SilverBitcoin RPC` adopts `DPoS` consensus mechanism with low transaction cost, low transaction latency, high transaction concurrency, and supports up to 10,000 validators.
...
SilverBitcoin RPC implements a four-tier validator system based on staking amounts:

### Bronze Tier (Entry Level)
- **Minimum Stake:** 1,000 SBTC
...
SilverBitcoin RPC uses a transparent and fair fee distribution model:
...
**No Burning:** Unlike many other blockchains, SilverBitcoin RPC does not burn any tokens, ensuring all fees contribute to network security and development.
...
- **epoch:** Time interval in blocks, currently 1 epoch = 50 blocks on `SilverBitcoin RPC`. At the end of each epoch, the blockchain interacts with the system contracts to update active validators.
...
- **Bronze Tier:** 1,000 SBTC minimum
- **Silver Tier:** 10,000 SBTC minimum  
- **Gold Tier:** 100,000 SBTC minimum
- **Platinum Tier:** 1,000,000 SBTC minimum
...
1. Ensure you have the minimum staking amount (1,000 SBTC for Bronze tier)
