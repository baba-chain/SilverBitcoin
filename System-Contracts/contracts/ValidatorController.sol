...
contract ValidatorController is Ownable {
    ...
    uint256 public silverbitcoinPriceUSD = 38; // Price in cents (0.38 USD = 38 cents)
    ...
    struct ValidatorTier {
        uint256 minStakingAmount;    // Minimum staking amount in SBTC (18 decimals)
    ...
    constructor() {
        ...
        // Initialize validator tiers with simplified staking amounts
        // Bronze Tier: 1,000 SBTC minimum
        validatorTiers[0] = ValidatorTier({
            minStakingAmount: 1000 * 1e18,    // 1,000 SBTC
        ...
        validatorTiers[1] = ValidatorTier({
            minStakingAmount: 10000 * 1e18,   // 10,000 SBTC
        ...
        validatorTiers[2] = ValidatorTier({
            minStakingAmount: 100000 * 1e18,  // 100,000 SBTC
        ...
        validatorTiers[3] = ValidatorTier({
            minStakingAmount: 1000000 * 1e18, // 1,000,000 SBTC
    ...
    function viewAnnualStakingReward(address validator) public view returns(uint256) {
        ...
        if (useTierBasedRewards) {
            ...
            // Convert tier reward from cents to SBTC tokens based on current price
            // tierRewardCents / silverbitcoinPriceUSD = annual reward in SBTC
            // Then calculate pro-rated amount based on time since last claim
            annualReward = (tierRewardCents * 1e18 * timeSinceLastClaim) / (silverbitcoinPriceUSD * SECONDS_PER_YEAR);
        } else {
    ...
    */
    ...
    function updateSilverBitcoinPrice(uint256 newPriceInCents) external {
        ...
        silverbitcoinPriceUSD = newPriceInCents;
    ...
    function calculateDollarValueOfReward(address validator) external view returns (uint256 dollarValueInCents) {
        ...
        dollarValueInCents = (tokenReward * silverbitcoinPriceUSD) / 1e18;
