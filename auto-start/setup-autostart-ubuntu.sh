...
if [ -d "$ACTUAL_HOME/Desktop/SilverBitcoin" ]; then
    ...
    echo -e "${RED}❌ SilverBitcoin directory not found!${NC}"
    ...
    echo ""
    read -p "Enter full path to SilverBitcoin directory: " BLOCKCHAIN_DIR
...
if [ ! -f "$BLOCKCHAIN_DIR/start-all-nodes.sh" ]; then
    echo -e "${RED}❌ start-all-nodes.sh not found in $BLOCKCHAIN_DIR${NC}"
    exit 1
fi

if [ ! -f "$BLOCKCHAIN_DIR/stop-all-nodes.sh" ]; then
    echo -e "${RED}❌ stop-all-nodes.sh not found in $BLOCKCHAIN_DIR${NC}"
    exit 1
fi
...
Documentation=https://github.com/baba-chain/SilverBitcoin
...
# Start all nodes
ExecStart=$BLOCKCHAIN_DIR/start-all-nodes.sh

# Stop all nodes
ExecStop=$BLOCKCHAIN_DIR/stop-all-nodes.sh
